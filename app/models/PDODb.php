...
class PDODb
{
    ...
    public function connect()
    {
        ...
        if ($dbType == 'sqlite') {
            ...
            // Assuming MySQL settings
            $this->connectionParams['type'] = 'mysql';  // Defaulting to MySQL
            $this->connectionParams['host'] = 'localhost';  // Docker service name
            $this->connectionParams['dbname'] = 'preschool';  // From your compose file
            $this->connectionParams['port'] = 3306;  // From your Docker Compose file
            $this->connectionParams['username'] = 'root';  // Root user as defined
            $this->connectionParams['password'] = 'Admin@1234';  // Password defined in environment
            $this->connectionParams['charset'] = 'utf8';  // Defaulting to UTF-8
            ...
            try {
                ...
                throw new Exception('Connection failed: ' . $e->getMessage() . ' (Host: ' . $this->connectionParams['host'] . ', DB: ' . $this->connectionParams['dbname'] . ', Port: ' . $this->connectionParams['port'] . ')');
            }
            
            // Set charset if provided
            if (!empty($this->connectionParams['charset'])) {
                $this->pdo->query("SET NAMES '{$this->connectionParams['charset']}'");
            }
        }
    }
